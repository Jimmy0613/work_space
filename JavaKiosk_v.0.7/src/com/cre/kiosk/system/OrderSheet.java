...
public class OrderSheet {
	...
	boolean order = true; //취소되면 false로 바꿈.
	int sumPrice;
	...
	public static void getOrderSheet(Order[] orderList) {
		...
		strOrder = Cw.sf("상품") + Cw.sf("수량") + Cw.sf("가격") + "\n" + Cw.lineStr() + strOrder + Cw.lineStr() + Cw.sf("총액")
				+ Cw.sf("") + Cw.sf(Cw.df(sumPrice) + "원");
	...
	public static void getOrderSheetByNumber(int listNum) {
		...
		Cw.lineWn("<주문번호 -" + (listNum + 1) + "- > (0:이전으로 x:주문취소)");
	...
	public static void cancelOrder(int listNum) {
		...
		case "x":
			...
			break;
		case "o":
			orderSheetList.get(listNum).orders = "(취소됨)" + "\n" + Cw.lineStr() + orderSheetList.get(listNum).orders;
