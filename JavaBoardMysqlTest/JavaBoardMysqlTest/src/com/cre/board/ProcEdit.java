...
public class ProcEdit {
	public static void run() {
		...
		while (true) {
			...
			case "o":
				if(title.equals("")) {
					db.dbEdit("update board_test set content='" + content + "' where post_num = " + ProcList.postNum);
				} else if(content.equals("")) {
					db.dbEdit("update board_test set title='" + title +"' where post_num = " + ProcList.postNum);
				} else {
					db.dbEdit("update board_test set title='" + title +"', content='" + content + "' where post_num = " + ProcList.postNum);
				}
				Cw.wn("수정했습니다.");
