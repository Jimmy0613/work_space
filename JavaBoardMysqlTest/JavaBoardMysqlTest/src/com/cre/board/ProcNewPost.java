		...
		post: while (true) {
			...
			while (true) {
				...
				case "o":
					date = now.toString();
					if (title.equals("")) {
						db.dbWrite("insert into board_test (wr_id, content, date) values ('" + writer + "', '" + content
								+ "', '" + date + "');");
					} else if (writer.equals("")) {
						db.dbWrite("insert into board_test (title, content, date) values ('" + title + "', '" + content
								+ "', '" + date + "');");
					} else if (writer.equals("x") && title.equals("x")) {
						db.dbWrite(
								"insert into board_test (content, date) values ('" + content + "', '" + date + "');");
					} else {
						db.dbWrite("insert into board_test (title, wr_id, content, date) values ('" + title + "', '"
								+ writer + "', '" + content + "', '" + date + "');");
					}
					break post;
				case "x":
