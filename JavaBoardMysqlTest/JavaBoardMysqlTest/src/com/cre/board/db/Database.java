...
public class Database {
	...
	public void dbRead(int postNum) {
		dbHit(postNum);
		try {
			result = st.executeQuery("select * from board_test where post_num = " + postNum);
			if (result.next()) {
				do {
	...
	public void dbList() {
		dbPost();
		try {
			result = st.executeQuery("select * from board_test");
	...
	public void dbDel(int postNum) {
		dbPost();
		try {
			st.executeUpdate("delete from board_test where post_num =" + postNum);
	...
	public void dbHit(int postNum) {
		dbPost();
		try {
			st.executeUpdate("update board_test set hit = hit + 1 where post_num =" + postNum);
		} catch (SQLException e) {
			e.printStackTrace();
		}
